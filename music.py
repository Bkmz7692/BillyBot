...
class Song:
    ...
    def create_embed(self):
        embed = (discord.Embed(title='I cant believe this story, what you tell me. Сейчас играет ',
                               description='```css\n{0.source.title}\n```'.format(self),
                               color=discord.Color.blurple())                 
                 .add_field(name='Включил', value=self.requester.mention)
                 .add_field(name='Автор видеопленки', value='[{0.source.uploader}]({0.source.uploader_url})'.format(self))
...
class Music(commands.Cog):
    ...
    async def cog_command_error(self, ctx: commands.Context, error: commands.CommandError):
        await ctx.send('Fuck You. Произошла какая-то ошибка: {}'.format(str(error)))
    ...
    async def _queue(self, ctx: commands.Context, *, page: int = 1):
        ...
        if len(ctx.voice_state.songs) == 0:
            return await ctx.send('В очереди нет треков, Jabrony.')
    ...
    async def ensure_voice_state(self, ctx: commands.Context):
        if not ctx.author.voice or not ctx.author.voice.channel:
            raise commands.CommandError('Сначала зайди в Gym.')

        if ctx.voice_client:
            if ctx.voice_client.channel != ctx.author.voice.channel:
                raise commands.CommandError('Billy уже там.')


bot = commands.Bot('-', description='Best bot in this Gym.')
...
bot.run('OTg3NDE3NDUwODMwMjU4MjY3.GwNTgU.ZDZfU7_bHayJJWix84jUMct9G7bpBDdbR8aVTY')
